...
if($_GET['id'] == "") {
  ...
  $query=$mysqli->query("SELECT * FROM soh_child where id= '$ChildID'");
   $row = $query->fetch_assoc();//mysql_fetch_assoc($query);
  $ChildName=$row[name];
  $ChildAge=$row[age];  
  $ChildTalent=$row[talent];
  $ChildDescription=$row[description];  
  $ChildImage=$row[imagePath];
  $ChildClass=$row[childClassData];
}
?>
<!DOCTYPE html>
...
<!--==============================Content=================================-->
		<div class="content">
			...
			<div class="menu_block">
						...
						<nav class="horizontal-nav full-width horizontalNav-notprocessed">
							<ul class="sf-menu">
								<li ><a href="child_list.php">View Child</a></li>								
							</ul>
						</nav>
						<div class="clear"></div>
					</div>
				...
				<div class="grid_12">
					...
					<form id="form" action="update_child_action.php" method="post" enctype="multipart/form-data">
						...
						<label class="email">
							<input type="text" value="<?php echo htmlspecialchars($ChildAge); ?>" name="childAge" placeholder="age:" data-constraints='@Required' required />
							<span class="empty-message">*This field is required.</span>
							<span class="error-message">*This is not a valid email.</span>
						</label>
						<label class="email">
							<input type="text" value="<?php echo htmlspecialchars($ChildTalent); ?>" name="childTalent" placeholder="talent:" data-constraints='@Required' required />
							<span class="empty-message">*This field is required.</span>
							<span class="error-message">*This is not a valid email.</span>
						</label>
						<label class="email">
							<input type="text" value="<?php echo htmlspecialchars($ChildClass); ?>" name="childClass" placeholder="Class:" data-constraints='@Required' required />
							<span class="empty-message">*This field is required.</span>
							<span class="error-message">*This is not a valid email.</span>
						</label>
