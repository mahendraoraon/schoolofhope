...
if($_GET['id'] == "") {
  ...
  $query=$mysqli->query("SELECT * FROM soh_news where id= '$NewsID'");
   $row = $query->fetch_assoc();//mysql_fetch_assoc($query);   
  $NewsTitle=$row[newsTitle];
  $NewsDescription=$row[newsText];  
}
?>
<!DOCTYPE html>
<html lang="en">
...
<!--==============================Content=================================-->
		<div class="content">
			...
			<div class="menu_block">
						...
						<nav class="horizontal-nav full-width horizontalNav-notprocessed">
							<ul class="sf-menu">
								<li class="current"><a href="newslist.php">View News</a></li>								
							</ul>
						</nav>
						<div class="clear"></div>
					</div>
...
<input type="hidden" value="<?php echo htmlspecialchars($NewsID); ?>" name="Newsid" id="NewsID" />

						<label class="name">
							<input type="text" value="<?php echo htmlspecialchars($NewsTitle); ?>" placeholder="Title:" name="news_title" Required />
						</label>
					
						<label class="message">
							<textarea placeholder="Description:" name="news_text" required ><?php echo htmlspecialchars($NewsDescription); ?></textarea>
